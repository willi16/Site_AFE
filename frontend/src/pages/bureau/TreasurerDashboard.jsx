...
import { DollarSign, TrendingUp, TrendingDown, FileText, BarChart3, Clock } from 'lucide-react';
        ...
        <div className="flex gap-2 mb-6">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${activeTab === tab.key ? 'bg-primary-500 text-white' : 'bg-white text-surface-600 hover:bg-surface-50 border border-surface-100'}`}>
          ...
          <>
            {activeTab === 'overview' && (
              <div className="bg-white rounded-2xl border border-surface-100 p-6">
                ...
                {records.length === 0 ? (
                  <p className="text-surface-500 text-center py-8">Aucun enregistrement financier.</p>
                ) : (
            ...
            {activeTab === 'records' && (
              <div className="bg-white rounded-2xl border border-surface-100 p-6">
                ...
                {records.length === 0 ? (
                  <p className="text-surface-500 text-center py-8">Aucun enregistrement.</p>
                ) : (
                    ...
                    {records.map(r => (
                      <div key={r.id} className="flex items-center justify-between py-3 px-4 bg-surface-50 rounded-xl">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-surface-800">{r.title}</p>
                            <p className="text-xs text-surface-400">{r.category} - {r.date}</p>
                          </div>
                        </div>
                        <span className={`text-sm font-bold ${r.record_type === 'income' ? 'text-emerald-600' : 'text-red-600'}`}>
                          {r.record_type === 'income' ? '+' : '-'}{parseFloat(r.amount).toLocaleString('fr-FR')} FCFA
            ...
            {activeTab === 'reports' && (
              <div className="bg-white rounded-2xl border border-surface-100 p-6">
                ...
                {reports.length === 0 ? (
                  <p className="text-surface-500 text-center py-8">Aucun rapport de réunion.</p>
                ) : (
                  <div className="space-y-3">
                    {reports.map(r => (
